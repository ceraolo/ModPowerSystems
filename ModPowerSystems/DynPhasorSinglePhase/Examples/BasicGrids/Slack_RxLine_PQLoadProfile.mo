...
model Slack_RxLine_PQLoadProfile
  ...
  inner Base.System system(freq_nom = 60) annotation (
    Placement(transformation(origin = {32, -38}, extent = {{-70, 54}, {-50, 74}})));
...
ModPowerSystems.DynPhasorSinglePhase.Loads.PQLoadProfile PQ1(Vnom = 24e3, profileFileName = LoadProfileFile, profileName = LoadProfileName, smoothnessSetting = Modelica.Blocks.Types.Smoothness.ConstantSegments)  annotation (
...
equation
  ...
  annotation (
    ...
    experiment(StopTime= 6, Interval = 0.0001, StartTime = 4, Tolerance = 1e-06));
end Slack_RxLine_PQLoadProfile;
